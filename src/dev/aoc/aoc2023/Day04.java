...
public class Day04 extends Day {
    ...
    public Object solvePart1() {
        ...
        wins = input
                ...
                .toArray();
        // System.out.printf("Stats: avg wins %f%n", (double)Arrays.stream(wins).sum() / wins.length);
    ...
    private Object part2Long() {
        ...
        for (int w = 0; w < wins.length; w++) {
            int winCnt = wins[w];
            int ci = w % counts.length;
            for (int wc = 0; wc < winCnt; wc++) {
                if (w + 1 + wc >= wins.length) {
                ...
                if (counts[cic] < 0) {
                    System.out.printf("card %d overflow%n", w + 1 + wc);
                    return result;
                }
            }
            // result2 += counts2[w];
            ...
            counts[ci] = 1;
            if (result < 0) {
                System.out.printf("result at card %d 64-bit overflow%n", w);
    ...
    private Object part2BI() {
        ...
        for (int w = 0; w < wins.length; w++) {
            ...
            for (int wc = 0; wc < winCnt; wc++) {
                ...
                int cic = (w + 1 + wc) % counts.length;
                // counts[w + 1 + wc] += counts[w];
                counts[cic] = counts[cic].add(counts[ci]);
            }
            // result += counts[w];
            result = result.add(counts[ci]);
            counts[ci] = BigInteger.ONE;
        }
        return result;
    }
    ...
    private List<Scratchcard> inputParse(List<String> lines) {
        ...
        for (var line : lines) {
            ...
            var winningSet = Arrays.stream(line.substring(posWinStart, posWinEnd).split("\\s+")).collect(Collectors.toSet());
            // old pre-Set version
            // int[] winning = Arrays.stream(line.substring(posWinStart, posWinEnd).split(" "))
            //         .map(String::trim)
            //         .filter(s -> !s.isEmpty())
            ...
            var havingSet = Arrays.stream(line.substring(posWinEnd + 3).split("\\s+")).collect(Collectors.toSet());
            // old pre-Set version
            // int[] having = Arrays.stream(line.substring(posWinEnd + 3).split(" "))
            //         .map(String::trim)
            //         .filter(s -> !s.isEmpty())
            //         .map(Integer::parseInt)
            //         .sorted()
            //         .mapToInt(Integer::intValue)
            //         .toArray();
            cards.add(new Scratchcard(winningSet, havingSet));
            // manual intersection with marching indices
            // int w = 0, h = 0, points = 0;
            // while (w < winning.length && h < having.length) {
            //     if (winning[w] == having[h]) {
            //         if (puzzleA) {
